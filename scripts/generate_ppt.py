"""
SEMICON India Hackathon 2026 — Master Clean Presentation Generator
Team: SPARTANS (VSB Engineering College, Karur | Anna University)
Guaranteed:
- Single text frame per card (Zero duplicate textboxes)
- Perfectly sized cards with zero empty bottom space
- Direct PDF compilation & distribution to Downloads folder
"""
...
def build_master_deck():
    ...
    # 4 Member Cards on Right (2x2 Grid)
    members_data = [
    ...
    print(f"[SUCCESS] Clean Master PPTX created: {pptx_path}")
    return pptx_path


def export_pdf_clean(pptx_path):
    import comtypes.client

    pdf_local = pptx_path.parent / (pptx_path.stem + ".pdf")
    downloads_dir = pathlib.Path(os.environ["USERPROFILE"]) / "Downloads"

    try:
        ppt = comtypes.client.CreateObject("PowerPoint.Application")
        ppt.Visible = 1
        pres = ppt.Presentations.Open(str(pptx_path))
        time.sleep(1)
        pres.SaveAs(str(pdf_local), 32)
        try:
            pres.Close()
        except Exception:
            pass
        try:
            ...
            pass
        print(f"[SUCCESS] PDF Exported: {pdf_local}")
    except Exception as e:
        print(f"[WARNING] COM export: {e}")
    ...
    for dest_name in [
        ...
        src_file = pdf_local if dest_name.endswith(".pdf") else pptx_path
        if src_file.exists():
            shutil.copy2(str(src_file), str(downloads_dir / dest_name))
            print(f"[SUCCESS] Updated Downloads: {dest_name}")


if __name__ == "__main__":
    pptx = build_master_deck()
    export_pdf_clean(pptx)
